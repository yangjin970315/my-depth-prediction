...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = self.dataset_path +'/image/'+ self.paths['image'][idx]
        depth_path = self.dataset_path + '/data_depth_annotated/' + self.paths['depth'][idx]
        ...
        if self.mode == 'train':
            depth_completed_path = self.dataset_path + '/data_depth_annotated/' + self.paths['depth'][idx].replace("groundtruth","completed")
        ...
        if self.mode == 'train':
            ...
            depth_completed = depth_completed / 256.0
...
class KITTIDataModuleTest(LightningDataModule):
    def __init__(self, config: DictConfig):
        super().__init__()
        self.config = config
